module Firefly
  module CommandUtil
    ...
    # Replaces a specified keyword in a file with the contents of another file
    ...
    def self.replace_command(base_file, content_file, result_file, keyword)
      <<~command
        powershell -Command `
        "(gc -encoding Utf8 '#{base_file}') `
        -replace '#{keyword}', `
        (gc -encoding Utf8 -raw '#{content_file}') |`
        Out-File -encoding Utf8 #{result_file}"
      command
    end
  end
end
